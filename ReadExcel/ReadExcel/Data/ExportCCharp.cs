...
{
    internal class ExportCCharp : ExportBase
	{
		...
		public ExportCCharp(string worksheet, string clientTableName, string fileName) : base(worksheet, clientTableName,fileName){}
		...
		public override void Export(DataTable table, bool isSameTable)
		{
			...
			lines.Add($"#文件：{this.fileName}->工作表：{this.worksheet}->{tableFileName}");
			for (int i = 1; i < rows; i++)//行
			{
				...
				for (int j = 0; j < columns; j++)//列
				{
					...
					switch (i)
					{
						...
						case 3://服务端数据类型
							csDic[j].fieldType = strValue;
							if (csDic[j].IsListDictionary())
							{
								csDic[j].fieldType = csDic[j].fieldType.Replace("dictionary", "Hukiry").Replace("Dictionary", "Hukiry");
							}
							break;
						case 4://字段名称
						...
						default:
							...
							if (csDic[j].IsExport())
							{
								...
								else if (csDic[j].IsDictionary()) strValue = strValue.Replace(',', '_');
								else if (csDic[j].IsListDictionary()) strValue = strValue.Replace(',', '_');
								line += strValue + ",";
							}
							break;
					}
			...
			if (isSameTable)
			{
				...
				if (dictable[tableFileName].Count > 0)
					lines.InsertRange(1, dictable[tableFileName]);
			}
			this.CollectSameTable(tableFileName, temp);

			this.WritecsData(tableFileName, lines.ToArray());
		...
		{
			string dirPath = Path.Combine(ConstConfig.GetCCharpTxtDirPath.Replace('\\', '/'), "TableData");
			string filePathData = this.GetFilePath(dirPath, $"{fileName}.txt", false);
			File.WriteAllLines(filePathData, lines);
		}

		private void WriteCsHelp()
		{
			string tableFileName = $"Table{this.clientTableName}";

			string filePathHelp = this.GetFilePath(ConstConfig.GetCCharpDirPath, $"{tableFileName}.cs", false);
			string customCode = this.GetCustomCode(filePathHelp);

			List<string> lines = new List<string>();
			lines.Add("using System;");
			lines.Add("using System.Collections.Generic;");
			lines.Add("namespace Hukiry.Table");
			lines.Add("{");
			lines.Add($"	public class {tableFileName}:ITable");
			lines.Add("	{");
			...
			for (int i = 0; i < length; i++)
			{
				...
				if (i == 0)
				{
					lines.Add($"		public int Id=>this.{data.fieldName};");
				}
				lines.Add($"		///<summary>");
				...
				{
					if (data.IsListDictionary())
					{
						methodBody += this.SplitCode(data.fieldName, i, 3, data.GetTypeMethodCS());
					}
					else if (data.IsDictionary())
					{
						methodBody += this.SplitCode(data.fieldName, i, 2, data.GetTypeMethodCS());
					}
					else if (data.IsList())
					{
			...
			string filePathData = this.GetFilePath(ConstConfig.GetCCharpDirPath, $"{tableFileName}.cs", false);
		...
		private string SplitCode(string fieldName, int i, int listState, Hukiry<string, string, string, string> hukiry)
		{
			...
			{
				temp = $"			this.{fieldName} = new Dictionary<{hukiry.value1},{hukiry.value2}>();\n";
				temp += $"\t\t\tif (!string.IsNullOrEmpty(array[{i}]))\n";
				temp += "\t\t\t{\n";
				temp += $"\t\t\t\tstring[] lines{fieldName} = array[{i}].Split('|');\n";
				temp += $"\t\t\t\tfor (int i = 0; i < lines{fieldName}.Length; i++)\n";
				temp += "\t\t\t\t{\n";
				temp += $"\t\t\t\t\tstring[] dicArray = lines{fieldName}[i].Split('_');\n";

				if (hukiry.value1 == "bool" && hukiry.value2 != "bool")
					temp += $"\t\t\t\t\tthis.{fieldName}.Add({hukiry.value3}(dicArray[0]) == 1, {hukiry.value4}(dicArray[1]));\n";
				else if (hukiry.value1 != "bool" && hukiry.value2 == "bool")
					temp += $"\t\t\t\t\tthis.{fieldName}.Add({hukiry.value3}(dicArray[0]), {hukiry.value4}(dicArray[1]) == 1);\n";
				else if (hukiry.value1 == "bool" && hukiry.value2 == "bool")
					temp += $"\t\t\t\t\tthis.{fieldName}.Add({hukiry.value3}(dicArray[0]) == 1, {hukiry.value4}(dicArray[1]) == 1);\n";
				else
					temp += $"\t\t\t\t\tthis.{fieldName}.Add({hukiry.value3}(dicArray[0]),{hukiry.value4}(dicArray[1]));\n";
				temp += "\t\t\t\t}\n";
				temp += "\t\t\t}\n";
			}
			else
			{
				temp = $"			this.{fieldName} = new List<Hukiry<{hukiry.value1},{hukiry.value2}>>();\n";
				temp += $"\t\t\tif (!string.IsNullOrEmpty(array[{i}]))\n";
				temp += "\t\t\t{\n";
				temp += $"\t\t\t\tstring[] lines{fieldName} = array[{i}].Split('|');\n";
				temp += $"\t\t\t\tfor (int i = 0; i < lines{fieldName}.Length; i++)\n";
				...
				temp += $"\t\t\t\t\tstring[] dicArray = lines{fieldName}[i].Split('_');\n";
				if (hukiry.value1 == "bool" && hukiry.value2 != "bool")
					temp += $"\t\t\t\t\tthis.{fieldName}.Add(new Hukiry<{hukiry.value1},{hukiry.value2}>({hukiry.value3}(dicArray[0]) == 1, {hukiry.value4}(dicArray[1])));\n";
				else if (hukiry.value1 != "bool" && hukiry.value2 == "bool")
					temp += $"\t\t\t\t\tthis.{fieldName}.Add(new Hukiry<{hukiry.value1},{hukiry.value2}>({hukiry.value3}(dicArray[0]), {hukiry.value4}(dicArray[1]) == 1));\n";
				else if (hukiry.value1 == "bool" && hukiry.value2 == "bool")
					temp += $"\t\t\t\t\tthis.{fieldName}.Add(new Hukiry<{hukiry.value1},{hukiry.value2}>({hukiry.value3}(dicArray[0]) == 1, {hukiry.value4}(dicArray[1]) == 1));\n";
				else
					temp += $"\t\t\t\t\tthis.{fieldName}.Add(new Hukiry<{hukiry.value1},{hukiry.value2}>({hukiry.value3}(dicArray[0]),{hukiry.value4}(dicArray[1])));\n";
				temp += "\t\t\t\t}\n";
				temp += "\t\t\t}\n";
			}
